...
use super::progress::{Configuration, CandidacyStatus, Progress, ProgressSet, ProgressState};
...
impl<T: Storage> Raft<T> {
    ...
    fn add_voter_or_learner(&mut self, id: u64, learner: bool) {
        ...
        if (learner && self.prs().learner_ids().contains(&id))
            || (!learner && self.prs().voter_ids().contains(&id))
        {
            return;
        }
